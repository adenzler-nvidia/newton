...
- Support `<joint type="ball"/>` in the MJCF importer, and pass `joint_damping`/`joint_stiffness` through to the MuJoCo spec for ball joints (previously hardcoded to zero)
