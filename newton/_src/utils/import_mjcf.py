...
def parse_mjcf(
    ...
    # Get variable-length custom attributes for tendon parsing (frequency=None)
    ...
    # Only parse tendons if custom attributes are registered
    has_tendon_attrs = "mujoco:tendon_world" in builder.custom_attributes
    tendon_section = root.find("tendon")
    if tendon_section is not None and has_tendon_attrs:
        # Parse <fixed> elements - fixed tendons as linear combinations of joint positions
